...
if (isset ($_POST['go']) && $_POST['go']=='Poster') {
	...
	// si tout est bon, on peut commencer l'insertion dans la base
		...
		$base = mysql_connect ('http://localhost/phpmyadmin/db_structure.php?db=wiki', 'root', 'password');
		mysql_select_db ('wiki', $base) ;

		// on recupere la date de l'instant présent
		$date = date("Y-m-d H:i:s");

		// préparation de la requête d'insertion (table forum_reponses)
		$sql = 'INSERT INTO commentaire VALUES("", "'.mysql_escape_string($_POST['pseudo']).'", "'.mysql_escape_string($_POST['commentaire']).'", "'.$date.'", "'.$_GET['numero_du_sujet'].'")';
